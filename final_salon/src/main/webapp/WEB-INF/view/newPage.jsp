...
<body>
   <div class="listWrap">
      ...
      <div class="chong">
         <span id="56">총 <c:if test="${newLength<20 }">${newLength }</c:if><c:if test="${newLength>=20 }">20</c:if>개의 상품이 있습니다.</span> 
      </div>

      <div class="hr"></div>
      <div class="space3"></div>

        <c:if test="${newLength < 20}">
        <div id="doubleBox">
     	 <c:forEach items="${newList}" var="dto" begin="0" end="${newLength }" varStatus="status">
         <div class="box">
            <div class="image">
      ...
      <c:if test="${newLength >= 20}">
      <div id="doubleBox">
      <c:forEach items="${newList}" var="dto" begin="0" end="19" varStatus="status">
         <div class="box">
            <div class="image">
              <a href="detail.do?code=${dto.goods_code }" onclick="newproduct(this)"><img src="${dto.picture.goods_main }"/></a>
            </div>
